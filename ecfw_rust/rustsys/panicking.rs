...
pub extern "C" fn hard_fault_printer(regs: [u32; 8]) {
    ...
    print_async!("r0  = {:08x}  r1  = {:08x}  r2  = {:08x}  r3  = {:08x}\n",
             regs[0], regs[1], regs[2], regs[3]);
    print_async!("r12 = {:08x}  lr  = {:08x}  pc  = {:08x}  psr = {:08x}\n",
             regs[4], regs[5], regs[6], regs[7]);

    print_async!("\n");
    unsafe {
        print_async!("BFAR = {:08x}  CFSR = {:08x}  HFSR = {:08x}\n",
                 readmem(0xe000ed38),
                 readmem(0xe000ed28),
                 readmem(0xe000ed2c));
        print_async!("DFSR = {:08x}  AFSR = {:08x}  SCB_SHCSR = {:08x}\n",
                 readmem(0xe000ed30),
                 readmem(0xe000ed3c),
